...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setEnvironment(const GridMap::Ptr &map, const ColorDetector::Ptr detector_obj)
  {
    this->grid_map_ = map;
    this->dynamic_detector_ = detector_obj;
  }
  ...
  void BsplineOptimizer::calcMovingObjCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    ...
    /* use dynamic_detector_*/
    constexpr double a = 2.0, b = 1.0, inv_a2 = 1 / a / a, inv_b2 = 1 / b / b;

    for (int i = order_; i < end_idx; i++)
    {
      double time = ((double)(order_ - 1) / 2 + (i - order_ + 1)) * bspline_interval_;
      Eigen::Vector3d obj_prid = dynamic_detector_->evaluateEkf3DVel(t_now + time);
      Eigen::Vector3d dist_vec = cps_.points.col(i) - obj_prid;

      double ellip_dist = sqrt(dist_vec(2) * dist_vec(2) * inv_a2 + (dist_vec(0) * dist_vec(0) + dist_vec(1) * dist_vec(1)) * inv_b2);
      double dist_err = CLEARANCE - ellip_dist;

      Eigen::Vector3d dist_grad = cps_.points.col(i) - obj_prid;
      Eigen::Vector3d Coeff;
      Coeff(0) = -2 * (CLEARANCE / ellip_dist - 1) * inv_b2;
      Coeff(1) = Coeff(0);
      Coeff(2) = -2 * (CLEARANCE / ellip_dist - 1) * inv_a2;
...
bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    int restart_nums = 0, rebound_times = 0;
    ;
    bool flag_force_return, flag_occ, success, flag_occ_dynamic;
    new_lambda2_ = lambda2_;
    constexpr int MAX_RESART_NUMS_SET = 3;
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          if (dynamic_detector_->ifDetect()){
            Eigen::Vector3d obj_prid = dynamic_detector_->evaluateEkf3DVel(t - tm + t0.toSec());
            Eigen::Vector3d dist_vec = traj.evaluateDeBoorT(t) - obj_prid;
            double ellip_dist = sqrt(dist_vec(2) * dist_vec(2) * inv_a2 + (dist_vec(0) * dist_vec(0) + dist_vec(1) * dist_vec(1)) * inv_b2);
            flag_occ_dynamic = (ellip_dist < 1.1);
          }
...
//#define USE_SECOND_CLEARENCE_CHECK
      ...
      else
      {
        ...
        ((flag_occ || flag_occ_dynamic || ((min_ellip_dist_ != INIT_min_ellip_dist_) && (min_ellip_dist_ > swarm_clearance_))) && restart_nums < MAX_RESART_NUMS_SET) ||
        (flag_force_return && force_stop_type_ == STOP_FOR_REBOUND && rebound_times <= 20));
