...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/sequential_wait_time_ms", sequential_wait_time_ms_, -1);
    ...
    // broadcast_bspline_sub_ = nh.subscribe("planning/broadcast_bspline_from_planner", 100, &EGOReplanFSM::BroadcastBsplineCallback, this, ros::TransportHints().tcpNoDelay());

    bspline_pub_ = nh.advertise<traj_utils::Bspline>("planning/bspline", 10);
    data_disp_pub_ = nh.advertise<traj_utils::DataDisp>("planning/data_display", 100);
    change_vins_type_pub_ = nh.advertise<std_msgs::Bool>("/vins_cam_switch", 1);

    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    { 
      waypoint_sub_ = nh.subscribe("/hummingbird/goal", 1, &EGOReplanFSM::waypointCallback, this);
    }
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      ...
      std_msgs::Bool msg;
      msg.data = false;
      change_vins_type_pub_.publish(msg);

      planGlobalTrajbyGivenWps();
    }
    else if (target_type_ == TARGET_TYPE::GOAL_SEQUENCE){
      goal_sequence_sub_ = nh.subscribe("/goal_sequence", 1, &EGOReplanFSM::goalSequenceCallback, this);
    } else{
  ...
  void EGOReplanFSM::goalSequenceCallback(const traj_utils::GoalSequence &msg){
    double goal_num = msg.waypoint_num;
    std::vector<Eigen::Vector3d> wps(goal_num);
    for (int i = 0; i < goal_num; i++)
    {
      wps[i](0) = msg.waypoint_pts[i].x;
      wps[i](1) = msg.waypoint_pts[i].y;
      wps[i](2) = msg.waypoint_pts[i].z;
    }
    
    // the final goal is different according to the drone_id
    wps[goal_num-1](1) = wps[goal_num-1](1) + planner_manager_->pp_.drone_id * 2.0;
    ...
    std_msgs::Bool vins_msg;
    vins_msg.data = false;
    change_vins_type_pub_.publish(vins_msg);

    // plan Global Traj by GivenWps
    bool success = planner_manager_->planGlobalTrajWaypoints(odom_pos_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), wps, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());

    for (size_t i = 0; i < (size_t)goal_num; i++)
    {
      visualization_->displayGoalPoint(wps[i], Eigen::Vector4d(0, 0.5, 0.5, 1), 0.3, i);
      ros::Duration(0.001).sleep();
    }
  ...
  void EGOReplanFSM::swarmTrajsCallback(const traj_utils::MultiBsplinesPtr &msg)
  {
    ...
    int  count = 0;
    while (have_recv_pre_agent_==false 
           && count ++ < sequential_wait_time_ms_){
      ros::Duration(0.001).sleep();
    }
  ...
  void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  {
    ...
    for (double t = t_cur; t < info->duration_; t += time_step)
    {
      ...
      if (planner_manager_->dynamic_detector_->ifDetect()){
        Eigen::Vector3d obj_prid = planner_manager_->dynamic_detector_->evaluateEkf3DVel(t - t_cur + t_cur_global);
        Eigen::Vector3d dist_vec = info->position_traj_.evaluateDeBoorT(t) - obj_prid;
