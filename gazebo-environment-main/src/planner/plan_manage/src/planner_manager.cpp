...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    // obj_predictor_->init();
    // obj_pub_ = nh.advertise<visualization_msgs::Marker>("/dynamic/obj_prdi", 10); // zx-tod
    dynamic_detector_.reset(new ColorDetector);
    dynamic_detector_->init(nh);
    
    bspline_optimizer_.reset(new BsplineOptimizer);
    bspline_optimizer_->setParam(nh);
    // bspline_optimizer_->setEnvironment(grid_map_, obj_predictor_);
    bspline_optimizer_->setEnvironment(grid_map_, dynamic_detector_);
